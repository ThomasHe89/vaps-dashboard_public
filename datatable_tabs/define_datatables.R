#########################################################################
### Title:  Define data table objects used to render app.R
###
### Author: Hauke Licht
### Data:   August 4, 2016
...
# the following steps are repeated for each table represented in the "Data Tables" Tab of the app 
...
### DATATABLES ### 
  ...
  # Configuration Events
  configuration_ctr_yr_table <- merge(country[,1:3],mv_configuration_ctr_yr,by="ctr_id",all.y=T)
  configuration_ctr_yr_table <- configuration_ctr_yr_table[, setdiff(colnames(configuration_ctr_yr_table), c("type_of_change"))]
  
    # order table by ctr_id and sdate
    configuration_ctr_yr_table <- configuration_ctr_yr_table[order(configuration_ctr_yr_table$ctr_id, configuration_ctr_yr_table$year), ]
    
      # remove chunk from columns containing text/character
      clean_up_column <- ColumnsInTables[ColumnsInTables$table_name=="mv_configuration_ctr_yr" & ColumnsInTables$data_type %in% c("character varying", "text"), "column_name"]
      
        for (c in seq_along(clean_up_column)) { configuration_ctr_yr_table[, c ] <- gsub("\t|\t[.| ]","", configuration_ctr_yr_table[, c ] ) }
      
  # define list of variable choices for data table input/output
  var_choices_configuration_ctr_yr_table <- get_var_labels("configuration_ctr_yr_table")  
